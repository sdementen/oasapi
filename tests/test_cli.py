...
def test_validate_nok():
    runner = CliRunner()
    swagger_path = Path(__file__).parent.parent / "docs" / "samples" / "swagger_petstore_with_errors.json"
    result = runner.invoke(validate, [str(swagger_path)])

    assert result.output =="""The swagger is not valid. Following errors have been detected:
- Duplicate operationId @ '$['paths']['/pet/findByStatus']['get']' -> the operationId 'updatePet' is already used in an endpoint
...
- Security scope not found @ '$['paths']['/pet']['put']['security']['petstore_auth']['think:pets']' -> scope think:pets is not declared in the scopes of the securityDefinitions 'petstore_auth'
"""
